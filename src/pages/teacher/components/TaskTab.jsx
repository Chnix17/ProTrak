  ...
  const columns = [
    {
      ...
      width: '25%',
      render: (text, record) => {
        const isCompleted = record.project_task_is_done === 1;
        return (
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              <Text 
                strong 
                className={`text-gray-900 ${isCompleted ? 'line-through text-gray-400' : ''}`}
              >
                {text}
              </Text>
              {record.description && (
    ...
    {
      title: 'Status',
      dataIndex: 'project_task_is_done',
      key: 'status',
      width: '15%',
      render: (isDone) => {
        const status = isDone === 1 ? 'completed' : 'pending';
      ...
      <Row gutter={16}>
        <Col xs={24} sm={6}>
          <Card className="text-center border-l-4" style={{ borderLeftColor: '#faad14' }}>
            <div className="text-2xl font-bold text-gray-900">
              {tasks.filter(t => t.project_task_is_done === 0).length}
            </div>
        ...
        <Col xs={24} sm={6}>
          <Card className="text-center border-l-4" style={{ borderLeftColor: '#52c41a' }}>
            <div className="text-2xl font-bold text-gray-900">
              {tasks.filter(t => t.project_task_is_done === 1).length}
            </div>
